...
def balas_hammer(A, P, C, verbose=False):
    n = len(P)
    m = len(C)
...
def generate_random_transport_problem(n, max_cost=20, max_supply=100):
    """
    Génère un problème de transport carré n×n
    """
    A = [[random.randint(1, max_cost) for _ in range(n)] for _ in range(n)]

    P = [random.randint(10, max_supply) for _ in range(n)]
    C = [random.randint(10, max_supply) for _ in range(n)]
...
def cout_total(A, B):
    return sum(A[i][j] * B[i][j] for i in range(len(B)) for j in range(len(B[0])))
...
def benchmark_transport_graphic(N_values=[10, 40, 100], NB_RUNS=100):
    ...
    for n in N_values:
        for _ in range(NB_RUNS):
            A, P, C = generate_random_transport_problem(n)

            # -------- Nord-Ouest --------
            start = time.time()
            B_NO = nord_ouest(P, C)
            tNO = time.time() - start
            thetaNO = cout_total(A, B_NO)

            # -------- Balas-Hammer --------
            start = time.time()
            B_BH = balas_hammer(A, P, C, verbose=False)

            tBH = time.time() - start
            thetaBH = cout_total(A, B_BH)

            all_data['n'].append(n)
            all_data['thetaNO'].append(thetaNO)
            all_data['thetaBH'].append(thetaBH)
            all_data['tNO'].append(tNO)
            all_data['tBH'].append(tBH)
            all_data['thetaNO_tNO'].append(thetaNO + tNO)
            all_data['thetaBH_tBH'].append(thetaBH + tBH)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        while True:
            ...
            print("8 - Complexité totale des algorithmes")
            ...
            if choix_action == "0":
                ...
                data = benchmark_transport_graphic(
                    N_values=[10, 40, 100],
                    NB_RUNS=100
                )
                # N_values contient les différentes tailles de n
